...
function setup(){
 ...
 let freshId=0;
 const ctx=vm.createContext({current:()=>session,stations:[session],$:id=>nodes[id]??={},newEventId:()=>`fresh-${++freshId}`,
  ...
  renderEvidence(){},showPostCaptureSource(){},syncCaptureProductCodes(){},
 });
 for(const name of ['emptyWeighRound','resetWeighRound','roundHasData','hasPendingRoundData']){
  const line=script.split('\n').find(line=>line.startsWith('function '+name+'('));
  assert.ok(line,name);vm.runInContext(line,ctx);
 }
 let start=script.indexOf('function clearSavedRoundAfterLocalWrite(');
 vm.runInContext(script.slice(start,script.indexOf('\nasync function saveValidatedCapture(',start)),ctx);
 start=script.indexOf('async function saveValidatedCapture(){');
...
test('default save writes only the selected round and blocks duplicate clicks',async()=>{
 ...
 assert.equal(session.rounds[0].eventId,'fresh-1');
...
test('saving one completed pair keeps the other pair and its QR draft',async()=>{
 ...
 checks[0].resolve(false);await pending;
 assert.deepEqual(calls,['verify0','saveone','history']);
 assert.equal(session.rounds[0].saved,false);
 assert.equal(session.rounds[0].qr,'');
 assert.equal(session.rounds[0].coreImage,'');
...
test('the second round button writes only the second round',async()=>{
 ...
 assert.equal(session.rounds[1].saved,false);
 assert.equal(session.rounds[1].qr,'');
 assert.equal(session.rounds[0].qr,'QR1');
});
...
test('partial save failure retains the remaining round and releases save lock',async()=>{
 ...
 const pending=ctx.saveValidatedCapture(1);checks[1].resolve(false);await pending;
 assert.equal(session.rounds[0].saved,false);assert.equal(session.rounds[1].saved,undefined);
